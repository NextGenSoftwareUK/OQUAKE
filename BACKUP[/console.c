...
void Con_DrawInput (cb_context_t *cbx)
{
	...
	// draw input string with password masking for "star beamin" commands
	{
		char masked_line[MAXCMDLINE];
		int len = strlen (key_lines[edit_line] + ofs);
		int mask_start = -1, mask_end = -1;
		int j;
		
		// Check if this is "star beamin <user> <pass>" command
		if (len > 13 && !strncmp (key_lines[edit_line] + ofs, "star beamin ", 12))
		{
			const char *p = key_lines[edit_line] + ofs + 12;
			int spaces = 0;
			int pos = 12;
			
			// Find the password (third token after "star beamin")
			while (*p && spaces < 2 && pos < MAXCMDLINE - 1)
			{
				if (*p == ' ')
				{
					spaces++;
					if (spaces == 1)
						mask_start = pos + 1; // Start of password
				}
				else if (spaces == 1 && mask_start >= 0)
				{
					mask_end = pos + 1; // End of password
				}
				p++;
				pos++;
			}
		...
		// Build masked string
		q_strlcpy (masked_line, key_lines[edit_line] + ofs, sizeof (masked_line));
		if (mask_start >= 0 && mask_end > mask_start)
		{
			for (j = mask_start; j < mask_end && j < (int)sizeof (masked_line) - 1; j++)
				masked_line[j] = '*';
		}
		
		// Draw masked line
		for (i = 0; masked_line[i] && i < con_linewidth; i++)
			Draw_Character (cbx, (i + 1) << 3, vid.conheight - 16, masked_line[i]);
	}
