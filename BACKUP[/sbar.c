...
void Sbar_LoadPics (void)
{
	...
	sb_face_quad = Draw_PicFromWad ("face_quad");
		sb_face_anorak = Draw_TryCachePic ("gfx/face_anorak.png", TEXPREF_ALPHA | TEXPREF_PAD | TEXPREF_NOPICMIP);
		if (!sb_face_anorak)
			sb_face_anorak = Draw_TryCachePic ("face_anorak.png", TEXPREF_ALPHA | TEXPREF_PAD | TEXPREF_NOPICMIP);
		if (!sb_face_anorak)
			sb_face_anorak = Draw_TryCachePic ("gfx/anorak_face.png", TEXPREF_ALPHA | TEXPREF_PAD | TEXPREF_NOPICMIP);
		if (!sb_face_anorak)
			sb_face_anorak = Draw_TryCachePic ("anorak_face.png", TEXPREF_ALPHA | TEXPREF_PAD | TEXPREF_NOPICMIP);
		if (!sb_face_anorak)
			sb_face_anorak = Draw_PicFromWad ("face_anorak");
...
void Sbar_DrawFace (cb_context_t *cbx, int x, int y, qboolean classic_style)
{
	...
	if (OQuake_STAR_ShouldUseAnorakFace ())
	{
		if (sb_face_anorak && sb_face_anorak != pic_nul)
			Sbar_DrawPic (cbx, x, y, sb_face_anorak);
		else
			Sbar_DrawPic (cbx, x, y, sb_faces[4][0]);
		return;
	}
